...
add_filter('woocommerce_product_data_store_cpt_get_products_query', 'truelysell_exclude_listing_booking', 10, 2);

function truelysell_exclude_listing_booking($query, $query_vars)
{
...
//add_filter('woocommerce_short_description', 'truelysell_woocommerce_short_description', 10, 1);
function truelysell_woocommerce_short_description($post_excerpt)
{
    global $product;
    if ($product->get_type() == "property_package") {
        ...
        if (!$jobslimit) {
            $output .= "<li>";
            $output .= esc_html__('Unlimited number of listings', 'truelysell');
            $output .=  "</li>";
        } else {
            $output .= '<li>';
            $output .= esc_html__('This plan includes ', 'truelysell');
            $output .= sprintf(_n('%d listing', '%s listings', $jobslimit, 'truelysell') . ' ', $jobslimit);
            $output .= '</li>';

            $jobduration =  $product->get_duration();
            if (!empty($jobduration)) {
                $output .= '<li>';
                $output .= esc_html__('Listings are published ', 'truelysell');
                $output .= sprintf(_n('for %s day', 'for %s days', $product->get_duration(), 'truelysell'), $product->get_duration());
                $output .= '</li>';
            }
            $output .= "</ul>";
        }
...
add_filter('woocommerce_show_page_title', 'truelysell_hide_shop_title');
function truelysell_hide_shop_title()
{
    return false;
}
...
add_filter('woocommerce_output_related_products_args', 'truelysell_related_woo_per_page');

function truelysell_related_woo_per_page($args)
{
...
function truelysell_woocommerce_remove_item($truelysell_html, $cart_item_key)
{
    $cart_item_key = $cart_item_key;
    $truelysell_html = sprintf('<a href="%s" class="remove" title="%s"><i class="fa fa-times" aria-hidden="true"></i></a>', esc_url(wc_get_cart_remove_url($cart_item_key)), esc_html__('Remove this item', 'truelysell'));
    return $truelysell_html;
}

add_filter('woocommerce_cart_item_remove_link', 'truelysell_woocommerce_remove_item', 10, 2);


/**
 * Exclude products from a particular category on the shop page
 */
function truelysell_remove_packages_pre_get_posts_query($q)
{
...
add_action('woocommerce_product_query', 'truelysell_remove_packages_pre_get_posts_query');

function exclude_woocommerce_widget_product_categories($widget_args)
{
    $category = get_term_by('slug', 'truelysell-booking', 'product_cat');
...
add_filter('woocommerce_products_widget_query_args', function ($query_args) {
    ...
    $categories = array('truelysell-booking');
...
function woocommerce_header_add_to_cart_fragment($fragments)
{
    ...
    ob_start();
?>
    <div class="mini-cart-button"><i class="fa fa-shopping-cart"></i><span class="badge"><?php echo sprintf(_n('%d', '%d', WC()->cart->cart_contents_count, 'truelysell'), WC()->cart->cart_contents_count); ?></span></div>
...
?>
    <p class="woocommerce-mini-cart__total total">
        <strong><?php esc_html_e('Subtotal', 'truelysell'); ?></strong>
...
function woocommerce_header_add_to_cart_content_fragment($fragments)
{
    ...
    ob_start(); ?>
    <div class="truelysell-mini-cart">
                ...
                ?>
                        <li class="woocommerce-mini-cart-item <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
                            ...
                            <?php if (empty($product_permalink)) : ?>
                                <?php echo esc_html($thumbnail) . wp_kses_post($product_name); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                ?>
                            <?php else : ?>
                                <a href="<?php echo esc_url($product_permalink); ?>">
                                    <?php echo esc_html($thumbnail); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                    ?>
                                </a>
        ...
        <?php else : ?>
            ...
            <p class="woocommerce-mini-cart__empty-message"><?php esc_html_e('No products in the cart.', 'truelysell'); ?></p>
    ...
    <?php $fragments['div.truelysell-mini-cart'] = ob_get_clean();
    return $fragments;
}
