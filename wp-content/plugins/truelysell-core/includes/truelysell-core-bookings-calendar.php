...
/**
 * Truelysell_Core_Bookings class.
 */
class Truelysell_Core_Bookings_Calendar {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_truelysell_validate_coupon', array($this, 'ajax_validate_coupon'));
        add_action('wp_ajax_nopriv_truelysell_validate_coupon', array($this, 'ajax_validate_coupon'));
        
        add_action('wp_ajax_truelysell_calculate_booking_form_price', array($this, 'ajax_calculate_booking_form_price'));
        add_action('wp_ajax_nopriv_truelysell_calculate_booking_form_price', array($this, 'ajax_calculate_booking_form_price'));
        ...
        add_action('wp_ajax_truelysell_bookings_manage', array($this, 'ajax_truelysell_bookings_manage'));
        add_action('wp_ajax_truelysell_bookings_renew_booking', array($this, 'ajax_truelysell_bookings_renew_booking'));
 
        // booking page shortcode and post handling
        add_shortcode( 'truelysell_booking_confirmation', array( $this, 'truelysell_core_booking' ) );
        add_shortcode( 'truelysell_bookings', array( $this, 'truelysell_core_dashboard_bookings' ) );
        add_shortcode( 'truelysell_my_bookings', array( $this, 'truelysell_core_dashboard_my_bookings' ) );
        ...
        add_action( 'woocommerce_order_status_completed', array( $this, 'booking_paid' ), 9, 3 ); 
         // remove truelysell booking products from shop
        add_action( 'woocommerce_product_query', array($this,'truelysell_wc_pre_get_posts_query' ));  

        add_action( 'truelysell_core_check_for_expired_bookings', array( $this, 'check_for_expired_booking' ) );
    ...
    */
    public static function ajax_check_avaliabity( )  {
        ...
        if(isset($_POST['hour'])){
            ...
            if($_opening_hours_status){
                ...
                if(!empty($currentEndTime) && is_numeric(substr($currentEndTime, 0, 1)) ) {
                    if(substr($currentEndTime, -1)=='M'){
                        $currentEndTime = DateTime::createFromFormat('h:i A', $currentEndTime);
                        if($currentEndTime){
                            $currentEndTime = $currentEndTime->format('Hi');            
                        }

                        //
                    } else {
                        $currentEndTime = DateTime::createFromFormat('H:i', $currentEndTime);
                        if($currentEndTime){
                            $currentEndTime = $currentEndTime->format('Hi');
                        }
                    }
                    ...
                    if(is_array($storeSchedule['opens'])){
                            foreach ($storeSchedule['opens'] as $key => $start_time) {
                                ...
                                $sql = $wpdb->prepare(
                                    ...
                                    WHERE listing_id = %s AND DATE(date_start) = %s",
                                    $listing_id,
                                    $date
                                );
                                $data_book = $wpdb->get_results($sql, ARRAY_A);
                                $condition_met = true;
                                // $currentEndTime = ((int)$currentEndTime < (int)$start_time) ? 2400 + (int)$currentEndTime : (int)$currentEndTime;
                                    // $currentTime = ((int)$currentTime < (int)$start_time) ? 2400 + (int)$currentTime : (int)$currentTime;
                                    // $end_time = ((int)$end_time < (int)$start_time) ? 2400 + (int)$end_time : (int)$end_time;

                                foreach ($data_book as $row) {
                                    $date_start = $row['date_start'];
                                    $date_end = $row['date_end'];
                                    $timestamp_new = date('Y-m-d', strtotime( $date_start));
                                    $timestart = date('Hi', strtotime($date_start));
                                    $timeend = date('Hi', strtotime($date_end));
                                    // $highest_timeend = max($highest_timeend, $timeend);
                                    $timeend = ((int)$timeend < (int)$timestart) ? 2400 + (int)$timeend : (int)$timeend;
                                    

                                        if (
                                        (  
                                            (intval($timestart) >= (int)$currentTime && intval($timestart) < (int)$currentEndTime) || (intval($timeend) > (int)$currentTime && intval($timeend) <= (int)$currentEndTime)
                                        )
                                        )  {
                                                $condition_met = false;
                                                break;
                            ...
                            if ($condition_met && ((int)$currentTime != (int)$currentEndTime)) {

                                if (
                                    ($currentTime >= $start_time && $currentTime <= $end_time) &&
                                    ($currentEndTime >= $start_time && $currentEndTime <= $end_time)
                                ) {

                                        $ajax_out['free_places'] = 1;
                                        // $ajax_out['da'] = $currentEndTime;
                                }
                                else{
                                            $ajax_out['free_places'] = 0;
                                            // $ajax_out['da'] = $currentEndTime;
    ...
    public  function ajax_validate_coupon(){
        ...
        if(empty($coupon)){
            ...
            $ajax_out['message'] = esc_html__('Coupon was not provided','truelysell_core');
            wp_send_json( $ajax_out );
        }
        if(! self::check_if_coupon_exists($coupon) ){
            $ajax_out['error'] = true;
            $ajax_out['error_type'] = 'no_coupon_exists';
            $ajax_out['message'] = esc_html__('This coupon does not exist','truelysell_core');
            wp_send_json( $ajax_out );
        }
        ...
        if($wc_coupon->get_individual_use()){
            ...
            if(isset($coupons) && is_array($coupons) && count($coupons) > 1){
                ...
                $ajax_out['message'] =  __( 'This coupons can\'t be used with others.', 'truelysell_core' );
                wp_send_json( $ajax_out );
            }
        }
        ...
        if($wc_coupon->get_minimum_amount() > 0 && $wc_coupon->get_minimum_amount() > $price ) {
            ...
            $ajax_out['message'] = sprintf( __( 'The minimum spend for this coupon is %s.', 'truelysell_core' ), wc_price( $wc_coupon->get_maximum_amount() ) );
            wp_send_json( $ajax_out );
        }  
        ...
        if($wc_coupon->get_maximum_amount() > 0 && $wc_coupon->get_maximum_amount() < $price ) {
            ...
            $ajax_out['message'] = sprintf( __( 'The maximum spend for this coupon is %s.', 'truelysell_core' ), wc_price( $wc_coupon->get_maximum_amount() ) );
            wp_send_json( $ajax_out );
        }
        ...
        if($wc_coupon->get_usage_limit_per_user() && $user_id){
            ...
            if ( $usage_count >= $wc_coupon->get_usage_limit_per_user() ) {
               $ajax_out['error'] = true;
                ...
                $ajax_out['message'] = sprintf( __( 'Coupon usage limit has been reached', 'truelysell_core' ), wc_price( $wc_coupon->get_maximum_amount() ) );
                wp_send_json( $ajax_out );
            }   
        }
        ...
        if ( $wc_coupon->get_date_expires() &&  time() > $wc_coupon->get_date_expires()->getTimestamp()  ) {
                ...
                $ajax_out['message'] = sprintf( __( 'This coupon has expired.', 'truelysell_core' ), wc_price( $wc_coupon->get_maximum_amount() ) );
                 wp_send_json( $ajax_out );
        }
        ...
        if($wc_coupon->get_usage_limit()>0) {
            ...
            if ($usage_left > 0) {
                ...
                if($admins_coupon){
                   ...
                   $available_listings = $wc_coupon->get_meta('listing_ids');
                    ...
                    if(in_array($listing_id,$available_listings_array)) {
                        ...
                        $ajax_out['message'] =  esc_html__('This coupon is not applicable for this listing','truelysell_core');
                        wp_send_json( $ajax_out );
                    } 
                }
                ...
                $ajax_out['error'] = true;
                $ajax_out['error_type'] = 'coupon_limit_used';
                $ajax_out['message'] =  esc_html__('Coupon usage limit has been reached','truelysell_core');
                wp_send_json( $ajax_out );
            }  
    ...
    public static function ajax_calculate_price( ) {
        ...
        if(isset($_POST['services'])){
            ...
            if(isset($services) && !empty($services)){
                ...
                $bookable_services = truelysell_get_bookable_services($listing_id);
                ...
                foreach ($bookable_services as $key => $service) {
                    ...
                    if(in_array(sanitize_title($service['name']),array_column($services,'service'))) { 
                        ...
                        $services_price +=  truelysell_calculate_service_price($service, $tickets, 1, $countable[$i] );
                ...
                foreach( $new_slots as $number => $slot) { 
                    ...
                    <div class="time-slot" day="<?php echo $actual_day; ?>">
                        ...
                        <label for="<?php echo $actual_day.'|'.$number; ?>">
                            ...
                            <span><?php echo $slot[1]; esc_html_e(' slots available','truelysell_core') ?></span>
                        </label>
                    </div>
                    <?php } 
    ...
    public static function ajax_truelysell_bookings_renew_booking() {
    ...
    *
    */
    public static function ajax_truelysell_bookings_manage(  )  {
        ...
        if ( $_POST['dashboard_type'] != 'user' ){
            if($_POST['date_start']==''){
                ...
                $bookings_max_number = truelysell_count_bookings(get_current_user_id(),$args['status']);    
            } else {
...
//        if user dont have listings show his reservations
        if ( isset( $_POST['dashboard_type']) && $_POST['dashboard_type'] == 'user' ) {
            ...
            if($_POST['date_start']==''){
                $ajax_out = self :: get_newest_bookings( $args, $limit, $offset ); 
                $bookings_max_number = truelysell_count_my_bookings(get_current_user_id(),$args['status']);    
            } else {
                $ajax_out = self :: get_bookings( $_POST['date_start'], $_POST['date_end'], $args, 'booking_date', $limit, $offset, 'users' );    
                $bookings_max_number = self :: get_bookings_max( $_POST['date_start'], $_POST['date_end'], $args, 'booking_date');
            }
        ...
        $result['pagination'] = truelysell_core_ajax_pagination( $max_number_pages, absint( $_POST['page'] ) );
    ...
    public static function insert_booking( $args )  {
        ...
        if ( isset( $args['price'] ) ) $insert_data['price'] = $args['price'];
         ...
         $insert_id_get = $wpdb -> insert_id;
        
         


        $booking_data = $wpdb -> get_row( 'SELECT * FROM `'  . $wpdb->prefix .  'bookings_calendar` WHERE `id`=' . esc_sql( $insert_id_get ), 'ARRAY_A' );
        ...
        $insert_data_one = array(
            ...
            'expiring' => $booking_data['expiring'],
            'price' => $booking_data['price'],
            'bk_id' => $booking_data['ID'],     
        );

        $wpdb -> insert( $wpdb->prefix . 'truelysell_core_notification', $insert_data_one );
        return  $insert_id_get;
    }
    ...
    public static function set_booking_status( $booking_id, $status ) {
        ...
        switch ( $status ) 
        {
            ...
            case 'waiting' :
                ...
                do_action('truelysell_mail_to_user_waiting_approval',$mail_to_user_args);
                ...
                do_action('truelysell_mail_to_owner_new_reservation',$mail_to_owner_args);
            ...
            case 'confirmed' :
                ...
                $default_booking_expiration_time = truelysell_fl_framework_getoptions('default_booking_expiration_time');
                ...
                $_payment_option = get_post_meta($booking_data['listing_id'], '_payment_option', true);
                ...
                if($instant_booking) {
                    ...
                    do_action('truelysell_mail_to_user_instant_approval',$mail_to_user_args);
                    ...
                    do_action('truelysell_mail_to_owner_new_instant_reservation',$mail_to_owner_args);
                ...
                if($_payment_option == 'pay_cash') {
                    ...
                    do_action('truelysell_mail_to_user_pay_cash_confirmed', $mail_args);

                    $update_values['status'] = 'paid_cash';
                ...
                if ( $booking_data['price'] == 0 )
                {
                    ...
                    do_action('truelysell_mail_to_user_free_confirmed',$mail_args);
                ...
                // creating woocommerce order
                    ...
                    if(isset($comment->service)){
                        ...
                        $order->update_meta_data('truelysell_services', $comment->service);
                    }
                ...
                $update_values['status'] = $status;
                 ...
                 $mail_args = array(
                    ...
                    do_action('truelysell_mail_to_user_pay',$mail_args);
            ...
            case 'paid' :
                ...
                do_action('truelysell_mail_to_owner_paid',$mail_to_owner_args);
                ...
                do_action('truelysell_mail_to_user_paid',$mail_to_user_args);
                 // mail for user
            ...
            case 'cancelled' :
                ...
                do_action('truelysell_mail_to_user_canceled',$mail_to_user_args);
                // delete order if exist
                if ( $booking_data['order_id'] )
                {
                    $order = wc_get_order( $booking_data['order_id'] );
                    $order->update_status( 'cancelled', __( 'Order is cancelled.', 'truelysell_core' ) );
                }
                $comment = json_decode($booking_data['comment']);
        ...
        $wpdb_returnsva = $wpdb -> update( $wpdb->prefix . 'bookings_calendar', $update_values, array( 'id' => $booking_id ) );

        $booking_data = $wpdb -> get_row( 'SELECT * FROM `'  . $wpdb->prefix .  'bookings_calendar` WHERE `id`=' . esc_sql( $booking_id ), 'ARRAY_A' );
         
       
         $update_values_one['rstatus'] = 'unread';
         $update_values_one['bookings_author'] = $booking_data['bookings_author'];
         $update_values_one['owner_id'] = $booking_data['owner_id'];
         $update_values_one['listing_id'] = $booking_data['listing_id'];
         $update_values_one['date_start'] = $booking_data['date_start'];
         $update_values_one['comment'] = $booking_data['comment'];
         $update_values_one['order_id'] = $booking_data['order_id'];
         $update_values_one['status'] = $booking_data['status'];
         $update_values_one['type'] = $booking_data['type'];
         $update_values_one['created'] = $booking_data['created'];
         $update_values_one['expiring'] = $booking_data['expiring'];
         $update_values_one['price'] = $booking_data['price'];

        $wpdb -> update( $wpdb->prefix . 'truelysell_core_notification', $update_values_one, array( 'bk_id' => $booking_id ) );

        return $wpdb_returnsva;
    ...
    */
    public static function calculate_price( $listing_id, $date_start, $date_end, $multiply = 1, $services = false, $coupon= false ) {
        ...
        // get all special prices between two dates from truelysell settings special prices
        ...
        // get normal prices from truelysell listing settings
        $normal_price = (float) get_post_meta ( $listing_id, '_normal_price', true);
        ...
        if($listing_type == 'event'){
            if(isset($services) && !empty($services)){
                ...
                foreach ($bookable_services as $key => $service) {
                    ...
                    if(in_array(sanitize_title($service['name']),array_column($services,'service'))) { 
                        $services_price +=  truelysell_calculate_service_price($service, $multiply, 1, $countable[$i] );
        ...
        if(isset($services) && !empty($services)){
            ...
            foreach ($bookable_services as $key => $service) {
                ...
                if(in_array(sanitize_title($service['name']),array_column($services,'service'))) { 
                    $services_price +=  truelysell_calculate_service_price($service, $multiply, $days_count, $countable[$i] );
        ...
        return apply_filters('truelysell_booking_price_calc',$price, $listing_id, $date_start, $date_end, $multiply , $services);
    ...
    * 
     */
    public static function truelysell_core_booking( ) {
        if(!isset($_POST['value'])){
            esc_html_e("You shouldn't be here :)",'truelysell_core');
            return;
        }
        ...
        if ( isset($_POST['confirmed']) )
        {
                ...
                foreach ($bookable_services as $key => $service) {
                    ...
                    if(in_array(sanitize_title($service['name']),array_column($services,'service'))) { 
                        ...
                        $comment_services[] =  array(
                            ...
                            'price' => truelysell_calculate_service_price($service, $guests, $days_count, $countable[$i] ) 
                        );
            ...
            if ( get_transient('truelysell_last_booking'.$_user_id) == $data['listing_id'] . ' ' . $data['date_start']. ' ' . $data['date_end'] )
            {
                $template_loader = new Truelysell_Core_Template_Loader;
                ...
                $template_loader->set_template_data( 
                    array( 
                        'error' => true,
                        'message' => __('Sorry, it looks like you\'ve already made that reservation', 'truelysell_core')
                    ) )->get_template_part( 'booking-success' ); 
                
                return;
            }

            set_transient( 'truelysell_last_booking'.$_user_id, $data['listing_id'] . ' ' . $data['date_start']. ' ' . $data['date_end'], 60 * 15 );
            ...
            switch ( $listing_meta['_listing_type'][0] ) 
            {
                case 'event' :
                    ...
                    $status = apply_filters( 'truelysell_event_default_status', 'waiting');
                    if($instant_booking == 'check_on' || $instant_booking == 'on' ) { 
                        $status = 'confirmed'; 
                        if(truelysell_fl_framework_getoptions('instant_booking_require_payment')){
                ...
                case 'rental' :
                    ...
                    // get default status
                    $status = apply_filters( 'truelysell_rental_default_status', 'waiting');
                    ...
                    if ( $free_places > 0 ) 
                    {
                        ...
                        if(truelysell_fl_framework_getoptions('instant_booking_require_payment') && $price > 0 ){
                            $status = "pay_to_confirm";
                        }}
                        $changed_status = self :: set_booking_status ( $booking_id, $status );
                     } else
                    {
                        ...
                        $error = true;
                        $message = __('Unfortunately those dates are not available anymore.', 'truelysell_core');
                ...
                case 'service' :
                    ...
                    $status = apply_filters( 'truelysell_service_default_status', 'waiting');
                    if($instant_booking == 'check_on' || $instant_booking == 'on') { $status = 'confirmed'; 
                        if(truelysell_fl_framework_getoptions('instant_booking_require_payment') ){
                            $status = "pay_to_confirm";
                        }
                    ...
                    if ( ! isset( $data['slot'] ) ) 
                    {
                        ...
                        $booking_id = self :: insert_booking ( array (
                            ...
                            'date_end' => $data['date_start'] . ' ' . $hour_end . ':00',
                            'comment' =>  json_encode ( array( 
                                'first_name'    => sanitize_text_field($_POST['firstname']),
                                'last_name'     => sanitize_text_field($_POST['lastname']),
                                'email'         => sanitize_email($_POST['email']),
                        ...
                        if ( $free_places > 0 ) 
                        {
                            ...
                            $booking_id = self :: insert_booking ( array (
                                ...
                                'date_end' => $data['date_start'] . ' ' . $hour_end,
                            ...
                            $status = apply_filters( 'truelysell_service_slots_default_status', 'waiting');
                            if($instant_booking == 'check_on' || $instant_booking == 'on') { $status = 'confirmed'; 
                         if(truelysell_fl_framework_getoptions('instant_booking_require_payment') && $price > 0 ){
                            $status = "pay_to_confirm";
                        }}
                         ...
                         } else
                        {
                            ...
                            $error = true;
                            $message = __('Those dates are not available.', 'truelysell_core');
            ...
            {
                $message = __( 'We have some technical problem, please try again later or contact administrator.', 'truelysell_core' );
                $error = true;
            }               
            ...
            switch ( $status )  {
                ...
                case 'waiting' :
                    ...
                    $message = esc_html__( 'Your booking is waiting for confirmation.', 'truelysell_core' );

                    break;

                case 'confirmed' :
                    if($price > 0){
                        $message = esc_html__( 'We are waiting for your payment.', 'truelysell_core' );
                    } else {
                        $message = '';
                    }
                ...
                case 'cancelled' :
                    ...
                    $message = esc_html__( 'Your booking was cancelled', 'truelysell_core' );
    ...
    public static function save_temp_reservation( $atts ) {
        ...
        $expired_date = date( 'Y-m-d H:i:s', strtotime( '+' . apply_filters( 'truelysell_expiration_booking_minutes', 15) . ' minutes', time() ) );
        ...
        set_transient( $atts['listing_id'], $temp_reservations, apply_filters( 'truelysell_expiration_minutes', 15) * 60 );
    ...
    * 
     */
    public static function truelysell_core_dashboard_bookings( ) {
    
          
        $users = new Truelysell_Core_Users;
        ...
        if(isset($_GET['status']) ){
            $booking_max = truelysell_count_bookings(get_current_user_id(),$_GET['status']); 
    ...
    public static function truelysell_core_dashboard_my_bookings( ) {
        ...
        $args =  apply_filters( 'truelysell_core_my_bookings_args', $args_default);

     
        $limit =  get_option('posts_per_page');

        $bookings = self :: get_newest_bookings($args,$limit );
        $booking_max = truelysell_count_my_bookings(get_current_user_id());
        $pages = ceil($booking_max/$limit);
        $template_loader = new Truelysell_Core_Template_Loader;
        $template_loader->set_template_data( 
            array( 
    ...
    * 
    ...
    public function truelysell_wc_pre_get_posts_query( $q ) {
    ...
    public function check_for_expired_booking(){
        ...
        if ( $bookings_ids ) {
            foreach ( $bookings_ids as $booking ) {
                ...
                do_action('truelysell_expire_booking',$booking);
            }
        }
