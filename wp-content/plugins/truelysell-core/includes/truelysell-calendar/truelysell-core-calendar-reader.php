...
class Truelysell_Core_iCal_Reader {
	...
	/**
	 * Creates the Truelysell_Core_iCal_Reader object
	 *
	 * @param mixed $files
	 * @param array $options
	 *
	 ...
	 * @param string $string
	 *
	 * @return Truelysell_Core_iCal_Reader
	 *
	 * @throws Exception
	 */
	public function init_string( string $string ): Truelysell_Core_iCal_Reader {
		...
		if ( empty( $this->cal ) ) {
			...
			trigger_error( 'Truelysell_Core_iCal_Reader::initString: Calendar already initialised in constructor' );
		}
	 ...
	 * @throws Exception
	 */
	public function init_file( string $file ): Truelysell_Core_iCal_Reader {
		if ( empty( $this->cal ) ) {
			$lines = $this->file_or_url( $file );

			$this->init_lines( $lines );
		} else {
			trigger_error( 'Truelysell_Core_iCal_Reader::initFile: Calendar already initialised in constructor' );
		}
	 ...
	 * @throws Exception
	 */
	public function init_url( string $url, $username = null, $password = null, $user_agent = null, $accept_language = null ): Truelysell_Core_iCal_Reader {
	...
	protected function process_recurrences() {
		...
		foreach ( $events as $key => $an_event ) {
			...
			if ( isset( $rrules['BYDAY'] ) ) {
				...
				if ( ! in_array( $frequency, array( 'MONTHLY', 'YEARLY' ) ) ) {
					if ( ! array_reduce( $rrules['BYDAY'], $check_by_days, true ) ) {
						error_log( "Truelysell_Core_iCal_Reader::ProcessRecurrences: A {$frequency} RRULE may not contain BYDAY values with numeric prefixes" );

						continue;
					}
				} elseif ( $frequency === 'YEARLY' && ! empty( $rrules['BYWEEKNO'] ) ) {
					if ( ! array_reduce( $rrules['BYDAY'], $check_by_days, true ) ) {
						error_log( 'Truelysell_Core_iCal_Reader::ProcessRecurrences: A YEARLY RRULE with a BYWEEKNO part may not contain BYDAY values with numeric prefixes' );
			...
			if ( ! is_int( $this->default_span ) ) {
				trigger_error( 'Truelysell_Core_iCal_Reader::defaultSpan: User defined value is not an integer' );
			}
	...
	public function events(): array {
		...
		if ( ! empty( $array ) ) {
			foreach ( $array as $event ) {
				$events[] = new Truelysell_Core_ICal_Event_Reader( $event );
			}
		}
	...
	public function events_from_range( $range_start = null, $range_end = null ): array {
		...
		if ( ! is_null( $range_start ) ) {
			try {
				...
				error_log( "Truelysell_Core_iCal_Reader::events_from_range: Invalid date passed ({$range_start})" );
				$range_start = false;
			}
		} else {
		...
		if ( ! is_null( $range_end ) ) {
			try {
				...
				error_log( "Truelysell_Core_iCal_Reader::events_from_range: Invalid date passed ({$range_end})" );
				$range_end = false;
			}
		} else {
