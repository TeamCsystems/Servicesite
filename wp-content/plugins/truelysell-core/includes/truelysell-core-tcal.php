...
class Truelysell_Core_iCal {
    ...
    public function __construct() {
        ...
        Truelysell_Core_iCal::$bookings = new Truelysell_Core_Bookings_Calendar;
        ...
        if ( ! wp_next_scheduled( 'truelysell_update_booking_icals' ) ) {
            wp_schedule_event( time(), '30min', 'truelysell_update_booking_icals' );
        }

        add_action( 'truelysell_update_booking_icals', array( $this, 'truelysell_update_booking_icals' ) );
    ...
    function add_new_listing_ical() {
        ...
        if ( empty( $name ) || empty( $url ) || ! intval( $listing_id ) ) {
            $result['type']         = 'error';
            $result['notification'] = esc_html__( "Please fill the form fields", "truelysell_core" );
            wp_send_json( $result );
            die();
        }
        ...
        if ( ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
            ...
            $result['type']         = 'error';
            $result['notification'] = esc_html__( "Please provide valid URL", "truelysell_core" );
            wp_send_json( $result );
        ...
        if ( ! in_array( $extension[0], array( 'html', 'ical', 'ics', 'ifb', 'icalendar' ) ) ) {
            if ( strpos( $url, 'calendar' ) !== false || strpos( $url, 'accommodation_id' ) !== false ) {
                ...
                $result['type']         = 'error';
                $result['notification'] = esc_html__( "No valid iCal file recognized. Please import 'ical', 'ics', 'ifb' or 'icalendar' file", "truelysell_core" );
                wp_send_json( $result );
        ...
        $current_icals = get_post_meta( $listing_id, 'truelysell_ical_imports', true );

        if ( is_array( $current_icals ) ) {
            //todo check if the same link was already added
            if ( in_array( $name, array_column( $current_icals, 'name' ) ) ) {
                $result['type']         = 'error';
                $result['notification'] = esc_html__( "It look's like you've already calendar with that name", "truelysell_core" );
                wp_send_json( $result );
                ...
                $result['type']         = 'error';
                $result['notification'] = esc_html__( "It look's like you've already added that calendar URL", "truelysell_core" );
                wp_send_json( $result );
        ...
        $action = update_post_meta( $listing_id, 'truelysell_ical_imports', $current_icals );
        ...
        if ( $action ) {
            ...
            if ( 0 < $imported['imported'] ) {
                $imported_info = sprintf( _n( "We've successfully imported %s event", "We've successfully imported %s events", $imported['imported'], 'truelysell_core' ), $imported['imported'] );
            } else {
                $imported_info = esc_html__( "No events imported", "truelysell_core" );
            }
            ...
            $result['type']         = 'error';
            $result['notification'] = esc_html__( 'There was problem updating the field.', 'truelysell_core' );
    ...
    function add_remove_listing_ical() {
        ...
        if ( $removed ) {
            ...
            $removed_info = sprintf( _n( "We've successfully removed this calendar with %s event", "We've successfully removed this calendar with %s events", $removed, 'truelysell_core' ), $removed );

        } else {

            $removed_info = esc_html__( "Calendar was removed, no events deleted", "truelysell_core" );
    ...
    function refresh_listing_import_ical() {
        ...
        if ( ! empty( $listing_id ) || intval( $listing_id ) ) {
            ...
            $result['type']         = 'success';
            $result['notification'] = esc_html__( 'Events from calendars were imported', 'truelysell_core' );

        } else {

            $result['type']         = 'error';
            $result['notification'] = esc_html__( 'There was error with the import, please try again', 'truelysell_core' );
    ...
    public static function get_saved_icals( $listing_id ) {
        ...
        $icals_list = get_post_meta( $listing_id, 'truelysell_ical_imports', true );

        ob_start();

        if ( ! empty( $icals_list ) ) : ?>
            <h4><?php esc_html_e( 'Imported Calendars', 'truelysell_core' ); ?></h4>
            <ul>
                <?php
                $i = 0;
                foreach ( $icals_list as $key => $value ) { ?>
                    <li><span><?php echo esc_html( $value['name'] ); ?></span>
                        ...
                        <a href="#" data-listing-id="<?php echo esc_attr( $listing_id ); ?>"
                           ...
                           class="ical-remove"><?php esc_html_e( 'Remove', 'truelysell_core' ); ?></a>
                    </li>
                    <?php $i ++;
                } ?>
            </ul>
            <a href="#" data-listing-id="<?php echo esc_attr( $listing_id ); ?>"
               class="update-all-icals"><?php esc_html_e( 'Import manually all calendars now', 'truelysell_core' ); ?><i
                        class="tip"
                        data-tip-content="<?php esc_html_e( 'All calendars are automaticaly refreshed every 30 minutes', 'truelysell_core' ); ?>"></i></a>
        <?php
        endif;
    ...
    public static function get_ical_export_url( $id ) {
        ...
        $ical_page = get_option( 'truelysell_ical_page' );
    ...
    public static function generate_event( $value ) {
        ...
        if ( get_option( 'truelysell_ical_timezone' ) ) {
...
DESCRIPTION:" . truelysell_escape_string( $comment ) . "
DTSTART:" . $start_date . "
DTEND:" . $end_date . "
...
";
    ...
    function truelysell_update_booking_icals() {
    ...
    function import_bookings_from_ical( $arr, $listing_id ): array {
        ...
        try {
            $ical = new Truelysell_Core_iCal_Reader(
                [
                    $url,
                ]
            );
    ...
    public function update_service_booking_for_ical_event( int $listing_id, string $local_name, int $bookings_author, $ical_event, $force_update = false ): array {
        ...
        $listing_slots = Truelysell_Core_Bookings_Calendar::get_slots_from_meta( $listing_id );
        ...
        if ( true === $slot_exists ) {
            ...
            if ( true === $force_update ) {
                ...
                $slot_bookings = Truelysell_Core_Bookings_Calendar::get_slots_bookings(
     ...
     * @return int
     * @throws Exception
     * @see Truelysell_Core_iCal::parse_event_dates() for format
     *
     */
    ...
    public function get_available_service_listing_slots( $listing_id, $date_start, $date_end ) {
        ...
        foreach ( $listing_slots_for_day as $key => $slot ) {
            ...
            $result = Truelysell_Core_Bookings_Calendar::get_slots_bookings( $date_start, $date_end, array(
                'listing_id' => $listing_id,
                'type'       => 'reservation'
            ) );
        }
