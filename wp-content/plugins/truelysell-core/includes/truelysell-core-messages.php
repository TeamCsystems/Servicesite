...
if ( ! defined( 'ABSPATH' ) )
	exit;
/**
 * Truelysell_Core_Messages class
 */
class Truelysell_Core_Messages {

	public function __construct() {

		add_shortcode( 'truelysell_messages', array( $this, 'truelysell_messages' ) );
        add_action('wp_ajax_truelysell_send_message', array($this, 'send_message_ajax'));
        add_action('wp_ajax_truelysell_send_message_chat', array($this, 'send_message_ajax_chat'));
		add_action('wp_ajax_truelysell_get_conversation', array($this, 'get_conversation_ajax'));

        add_action( 'truelysell_core_check_for_new_messages', array( $this, 'check_for_new_messages' ) );
	}
    ...
    public function check_for_new_messages() {
       ...
       $conversation_ids = $wpdb->get_col( $wpdb->prepare( "
                SELECT id FROM {$wpdb->prefix}truelysell_core_conversations
                WHERE (read_user_1 = '0'
                OR read_user_2 = '0' )
        ...
        if ( $conversation_ids ) {
            foreach ( $conversation_ids as $conversation_id ) {
                ...
                do_action('truelysell_mail_to_user_new_message',$conversation_id);
            }
        }
	...
	public  function start_conversation( $args = 0 )  {
        ...
        if(isset($wpdb->insert_id)) {
            ...
            do_action('truelysell_mail_to_user_new_conversation',$mail_args);
        } else {
            $id = false;
        }
    ...
    public  function send_new_message( $args = 0 )  {
        ...
        $result =  $wpdb -> insert( $wpdb->prefix . 'truelysell_core_messages', array(
    ...
    public function send_message_ajax() {
        ...
        if($id) {
            $result['type'] = 'success';
            $result['message'] = __( 'Your message was successfully sent' , 'truelysell_core' );
        } else {
            $result['type'] = 'error';
            $result['message'] = __( 'Message couldn\'t be send' , 'truelysell_core' );
        }
    ...
    public function send_message_ajax_chat() {        
        ...
        if(empty($message)){
            $result['type'] = 'error';
            $result['message'] = __( 'Empty message' , 'truelysell_core' );
            $result = json_encode($result);
            echo $result;  
        ...
        if(empty($conversation_id)){
            $result['type'] = 'error';
            $result['message'] = __( 'Whoops, we have a problem' , 'truelysell_core' );
            $result = json_encode($result);
            echo $result;  
    ...
    public function delete_conversations( $conv_id ) {
        ...
        if($conversation){
            ...
            if($conversation[0]->user_1 == $user_id || $conversation[0]->user_2 == $user_id ){
                ...
                $result = $wpdb -> delete( $wpdb->prefix . 'truelysell_core_conversations', array( 'id' => $conv_id) );
                $wpdb -> delete( $wpdb->prefix . 'truelysell_core_messages', array( 'conversation_id' => $conv_id) );
                return $result;
...
?>
                <li class="media message-bubble <?php if($user_id == $message->sender_id ) echo esc_attr('sent'); else  echo esc_attr('received'); ?> d-flex">
    ...
    public  function get_single_conversation( $user_id, $conversation_id)  {
        ...
        $result  = $wpdb -> get_results( "
        SELECT * FROM `" . $wpdb->prefix . "truelysell_core_messages` 
	...
	*/
	 ...
	 */
	public function truelysell_messages( $atts ) {
		
		if ( ! is_user_logged_in() ) {
			return __( 'You need to be signed in to manage your messages.', 'truelysell_core' );
		}
        ...
        if( isset( $_GET["action"]) && $_GET["action"] == 'view' )  {
            ...
            if( isset( $_GET["action"]) && $_GET["action"] == 'delete' )  {
                if(isset( $_GET["conv_id"]) && !empty($_GET["conv_id"])) {
                    ...
                    if($delete) { ?>
                        <div class="notification success"><p><?php esc_html_e('Conversation was removed','truelysell_core'); ?></p></div>
                    <?php } else { ?>
                        <div class="notification error"><p><?php esc_html_e('Conversation couldn\'t be removed.','truelysell_core'); ?></p></div>
                    <?php }
                } 
            }
