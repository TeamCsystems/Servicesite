...
class Truelysell_Core_Payouts {
    ...
    public function add_menu_item() {
            ...
            $args = apply_filters( 'truelysell_core_commissions_menu_items', array(
                    'page_title' => __( 'Commissions', 'truelysell_core' ),
                    'menu_title' => __( 'Commissions', 'truelysell_core' ),
                    'capability' => 'edit_products',
                    'menu_slug'  => 'truelysell_payouts',
            ...
            add_submenu_page( 'truelysell_payouts', 'Payouts History', 'Payouts History', 'edit_products', 'truelysell_payouts_list', array( $this, 'payouts_details_page' ) );
    }
    ...
    public function make_payout(){
        ...
        if(isset($_POST['commission']) && !empty($_POST['commission'])){
            if(is_array($_POST['commission'])){
                foreach ($_POST['commission'] as $key => $value) {
                    $orders_list[] = $key;
                    $wpdb->update( $wpdb->prefix . "truelysell_core_commissions", array('status'=>'paid'), array( 'id' => $key ) );
                    $order_id = $wpdb->get_var( $wpdb->prepare( 
                        '
                            SELECT order_id 
                            FROM '.$wpdb->prefix.'truelysell_core_commissions
                            WHERE id = %s
                        ', 
                        $key
                    ) );
        ...
        $wpdb->insert( $wpdb->prefix . "truelysell_core_commissions_payouts", $args );

        if($wpdb->insert_id){
            echo '<div class="updated"><p>'.esc_html__('Payout was created.','truelysell_core').'</p></div>';
            $id = $wpdb->insert_id; 
            ...
            <div class="wrap">
                ...
                <ul>
                    <li><span><?php esc_html_e( 'Payment for', 'truelysell_core' ); ?></span> <?php echo $user_data->display_name; ?></li>
                    <li><span><?php esc_html_e('Payment date','truelysell_core'); ?>:</span> <?php echo date(get_option( 'date_format' ), strtotime($payout['date']));  ?></li>
                    <li><span><?php esc_html_e('Payment amount','truelysell_core'); ?>:</span>  <?php echo wc_price($payout['amount']); ?></li>
                    <li><span><?php esc_html_e('Payment method','truelysell_core'); ?>:</span> <?php echo $payment_method ; ?></li>
                    <li><span><?php esc_html_e('Payment details','truelysell_core'); ?>:</span>  
                ...
                <?php 
                $commission_class = new Truelysell_Core_Commissions;
                $commissions = array();
    ...
    public function commissions_details_page() {
        ...
        if(isset($_POST['submit_new_payout'])){
            ...
            if ($user_data->truelysell_core_payment_type === 'paypal'){
                $payment_method = 'PayPal';
            }else if ($user_data->truelysell_core_payment_type === 'PayPal Payout'){
                $payment_method = 'PayPal Payout';
            }else {
                $payment_method = 'Bank Transfer';
            }
            ...
            ?>
            <div class="wrap">
                <h2><?php esc_html_e( 'New Payment for', 'truelysell_core' ); ?> <?php echo $user_data->display_name; ?></h2>
                <div class="payout-make-box">
                ...
                <form method="POST"  id="truelysell-make-payout">
                    <input type="submit" name="submit_new_payout" id="submit" class="button button-primary" value="Make Payout">
                    <input type="hidden" name="action" value="truelysell_make_payout" />
                    <input type="hidden" name="user_id" value="<?php echo $user_id; ?>" />                    
                    <?php if(isset($user_data->truelysell_core_payment_type)) { 
                        $payment_method = $user_data->truelysell_core_payment_type; 
                    } else {
                    ...
                    <?php if($payment_method == 'paypal'): ?>
                        <input type="hidden" name="payment_details" value="<?php echo $user_data->truelysell_core_ppemail ?>">
                    <?php endif; ?> 
                    <?php if($payment_method == 'banktransfer'): ?>
                        <input type="hidden" name="payment_details" value="<?php echo esc_attr($user_data->truelysell_core_bank_details); ?>">
                    <?php endif; ?>
                        ...
                        foreach ($commissions as $commission) { 
                            ...
                            <tr>
                                ...
                                <?php if (truelysell_is_payout_active()): ?>
                                    ...
                                    <?php else: ?>
                                        <td><em><?php _e('First, you need to cancel automatic commission that was previously sent by using PayPal Payout. Then you can send the commission manually.', 'truelysell'); ?></em></td>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <td><input type="checkbox" checked="checked" name="commission[<?php echo $commission['id']; ?>]"></td>
                                <?php endif; ?>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                                <a class="button color cancel-payout-button" href="#" data-pp_item_id="<?php echo $commission['payout_item_id']; ?>" data-commission_id="<?php echo $commission['id']; ?>">
                                                    ...
                                                    <?php _e('Cancel PayOut', 'truelysell'); ?>
                                                </a>
                                            <?php endif; ?>
                                        </td>
                                    <?php else: ?>
                ...
                <input type="submit" name="submit_new_payout" id="submit" class="button button-primary" value="Make Payout">
                    ...
                    <div class="loader-div truelysell-hidden">
        ...
        <?php } 
            ...
            $balances_table = new Truelysell_Balances_List_Table();
    ...
    public function payouts_details_page() {
        ...
        <?php } else {
            ...
            $payouts_table = new Truelysell_Payouts_List_Table();
    ...
    function get_payout($id){
        ...
        global $wpdb;
        return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM ". $wpdb->prefix . "truelysell_core_commissions_payouts WHERE ID = %d", $id ), ARRAY_A );  
    }
    function get_payouts($args){
        ...
        global $wpdb;
            ...
            $default_args = array(
                ...
                'order'        => 'DESC',
                'fields'       => 'ids',
                'table'        => $wpdb->prefix . "truelysell_core_commissions_payouts"
            );
...
if ( ! class_exists( 'Truelysell_Payouts_List_Table' ) ) {
    /**
     *
     *
     ...
     *
     */
    class Truelysell_Payouts_List_Table extends WP_List_Table {
    /** Class constructor */
        public function __construct() {

            parent::__construct( [
                'singular' => __( 'Payout', 'truelysell-core' ), // singular name of the listed records
                'plural'   => __( 'Payouts', 'truelysell-core' ), // plural name of the listed records
        ...
        public function get_columns() {
            $columns = array(
                    ...
                    'user_id'        => __( 'User', 'truelysell_core' ),
                    'orders'         => __( 'Orders number', 'truelysell_core' ),
                    'payment_method' => __( 'Payment method', 'truelysell_core' ),
                    'amount'         => __( 'Amount', 'truelysell_core' ),
                    'date'           => __( 'Date', 'truelysell_core' ),
        ...
        public function column_default( $item, $column_name ){
            switch( $column_name ) {
                ...
                case 'actions':
                $url = admin_url( 'admin.php?page=truelysell_payouts_list');
                
                $payout_url = esc_url( add_query_arg( 'view_payout', $item['id'], $url ) );
               
                printf( '<a class="button-primary view" href="%1$s" data-tip="%2$s">%2$s</a>', $payout_url, __( 'View Details', 'truelysell_core' ) );
                break;
                default:
        ...
        function no_items() {
            _e( 'No payouts set.','truelysell_core' );
        }
