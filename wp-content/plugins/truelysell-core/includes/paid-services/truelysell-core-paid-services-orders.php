...
class Truelysell_Core_Orders {
	...
	public function order_paid( $order_id ) {
		...
		if ( get_post_meta( $order_id, 'truelysell_core_paid_listings_processed', true ) ) {
			return;
		}
		foreach ( $order->get_items() as $item ) {
			...
			if ( $product &&  $product->is_type( 'listing_package' ) && $order->get_customer_id() ) {
				...
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = truelysell_core_give_user_package( $order->get_customer_id(), $product->get_id(), $order_id );
				}
		...
		update_post_meta( $order_id, 'truelysell_core_paid_listings_processed', true );
	}
	...
	private function attach_package_listings( $item, $order, $user_package_id ) {
		...
		foreach ( $listing_ids as $listing_id ) {
			if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
				truelysell_core_approve_listing_with_package( $listing_id, $order->get_user_id(), $user_package_id );
		...
		foreach ( $order->get_items() as $item ) {
			if ( isset( $item['listing_id'] )  ) {
				switch ( get_post_status( $item['listing_id'] ) ) {
					case 'pending' :
						echo wpautop( sprintf( __( '<strong>%s</strong> has been submitted successfully and will be visible once approved.', 'truelysell_core' ), get_the_title( $item['listing_id'] ) ) );
					break;
					case 'pending_payment' :
					case 'expired' :
						echo wpautop( sprintf( __( '<strong>%s</strong> has been submitted successfully and will be visible once payment has been confirmed.', 'truelysell_core' ), get_the_title( $item['listing_id'] ) ) );
					break;
					default :
						echo wpautop( sprintf( __( '<strong>%s</strong> has been submitted successfully.', 'truelysell_core' ), get_the_title( $item['listing_id'] ) ) );
					break;
				}
			} 
		}// End foreach().
...
Truelysell_Core_Orders::get_instance();