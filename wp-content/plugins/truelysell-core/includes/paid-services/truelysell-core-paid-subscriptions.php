...
/**
* Truelysell_Core_Paid_Listings_Subscriptions
*/
class Truelysell_Core_Paid_Listings_Subscriptions {
	...
	private function get_listing_subscription_order_id( $listing_id ) {
		if ( 'listing' === get_post_type( $listing_id ) ) {
			...
			$user_package    = truelysell_core_get_user_package( $user_package_id );
		...
		if ( 'listing' === $post->post_type ) {
			...
			if ( $package_product_id ) {
				$subscription_type = $this->get_package_subscription_type( $package_product_id );
					...
					$new_count = $wpdb->get_var( $wpdb->prepare( "SELECT package_count FROM {$wpdb->prefix}truelysell_core_user_packages WHERE id = %d;", $package_id ) );
					$new_count --;
	...
	if ( $id > 0 ) {
		...
		if ( 'listing' === $post_type) {
			...
			if ( $package_product_id ) {
				...
				$package  = $wpdb->get_row( $wpdb->prepare( "SELECT package_count, package_limit FROM {$wpdb->prefix}truelysell_core_user_packages WHERE id = %d;", $package_id ) );
...
public function subscription_ended( $subscription, $paused = false ) {
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}truelysell_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) );
		...
		if ( $user_package ) {
			...
			$listing_ids = truelysell_core_get_listings_for_package( $user_package->id );
	...
	delete_post_meta(  $subscription->get_id(), 'truelysell_core_subscription_packages_processed' );
}
...
public function subscription_activated( $subscription ) {
	...
	if ( get_post_meta( $subscription->get_id(), 'truelysell_core_subscription_packages_processed', true ) ) {
		return;
	}
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		if ( $product->is_type( array( 'listing_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
			...
			for ( $i = 0; $i < $item['qty']; $i ++ ) {
				$user_package_id = truelysell_core_give_user_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
			...
			foreach ( $listing_ids as $listing_id ) {
				if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
					truelysell_core_approve_listing_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
	...
	update_post_meta( $subscription->get_id(), 'truelysell_core_subscription_packages_processed', true );
}
...
public function subscription_renewed( $subscription ) {
	...
	foreach ( $subscription->get_items() as $item ) {
		...
		if ( $user_package_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}truelysell_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $subscription->get_id(), $legacy_id, $item['product_id'] ) ) ) {
			foreach ( $user_package_ids as $user_package_id ) {
				$package = truelysell_core_get_user_package( $user_package_id );

				if ( $listing_ids = truelysell_core_get_listings_for_package( $user_package_id ) ) {
					foreach ( $listing_ids as $listing_id ) {
...
public function switch_package( $user_id, $new_subscription, $old_subscription ) {
	...
	$user_package      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}truelysell_core_user_packages WHERE order_id IN ( %d, %d ) AND product_id = %d;", $old_subscription->id, $legacy_id, $old_subscription->product_id ) );
	...
	if ( $user_package ) {
		...
		$switching_to_package_id = truelysell_core_give_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
...
Truelysell_Core_Paid_Listings_Subscriptions::get_instance();