...
/**
 * Truelysell_Core_Admin_Add_Package class.
 */
class Truelysell_Core_Admin_Add_Package {
	...
	public function __construct() {
		...
		if ( ! empty( $_POST['save_package'] ) && ! empty( $_POST['truelysell_core_paid_listings_packages_nonce'] ) && wp_verify_nonce( $_POST['truelysell_core_paid_listings_packages_nonce'], 'save' ) ) {
			$this->save();
		}
	}
	...
	public function form() {
		...
		if ( $this->package_id && ( $package = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}truelysell_core_user_packages WHERE id = %d;", $this->package_id ) ) ) ) {
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<label for="package_limit"><?php _e( 'Listing Limit', 'truelysell_core' ); ?></label>
					<img class="help_tip tips" data-tip="<?php _e( 'How many listings should this package allow the user to post?', 'truelysell_core' ); ?>" src="<?php echo WC()->plugin_url() ?>/assets/images/help.png" height="16" width="16">
				</th>
				<td>
					<input type="number" step="1" name="package_limit" id="package_limit" class="input-text regular-text" placeholder="<?php _e( 'Unlimited', 'truelysell_core' ); ?>" value="<?php echo esc_attr( $package_limit ); ?>" />
				</td>
			</tr>
			<tr>
				<th>
					<label for="package_count"><?php _e( 'Listing Count', 'truelysell_core' ); ?></label>
					<img class="help_tip tips" data-tip="<?php _e( 'How many listings has the user already posted with this package?', 'truelysell_core' ); ?>" src="<?php echo WC()->plugin_url() ?>/assets/images/help.png" height="16" width="16">
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_duration"><?php _e( 'Listing Duration', 'truelysell_core' ); ?></label>
					<img class="help_tip tips" data-tip="<?php _e( 'How many days should listings posted with this package be active?', 'truelysell_core' ); ?>" src="<?php echo WC()->plugin_url() ?>/assets/images/help.png" height="16" width="16">
				</th>
				<td>
					<input type="number" step="1" name="package_duration" id="package_duration" value="<?php echo esc_attr( $package_duration ); ?>" class="input-text regular-text" placeholder="<?php _e( 'Default', 'truelysell_core' ); ?>" />
				</td>
			</tr>
			<tr>
				<th>
					<label for="package_featured"><?php _e( 'Feature Listings?', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_booking_module"><?php _e( 'Allow Booking Module', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_reviews"><?php _e( 'Allow Reviews Module', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_social_links"><?php _e( 'Allow Social Links Module', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_opening_hours"><?php _e( 'Allow Opening Hours Module', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_video"><?php _e( 'Allow Video', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_coupons"><?php _e( 'Allow Coupons', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_gallery"><?php _e( 'Allow Gallery', 'truelysell_core' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="package_option_gallery_limit"><?php _e( 'Allow Gallery', 'truelysell_core' ); ?></label>
				</th>
				<td>
					<?php 
					if(empty($package_option_gallery_limit)) { $package_option_gallery_limit = truelysell_fl_framework_getoptions('max_files'); } ?>
			...
			<tr>
				<th>
					<label for="user_id"><?php _e( 'User', 'truelysell_core' ); ?></label>
				</th>
				<td>
					<?php
					wp_dropdown_users(
			...
			<tr>
				<th>
					<label for="product_id"><?php _e( 'Product', 'truelysell_core' ); ?></label>
					<img class="help_tip tips" data-tip="<?php _e( 'Optionally link this package to a product.', 'truelysell_core' ); ?>" src="<?php echo WC()->plugin_url() ?>/assets/images/help.png" height="16" width="16">
				</th>
				<td>
					<select name="product_id" class="wc-enhanced-select" data-allow_clear="true" data-placeholder="<?php _e( 'Choose a product&hellip;', 'truelysell_core' ) ?>" style="width:25em">
						<?php
			...
			<tr>
				<th>
					<label for="order_id"><?php _e( 'Order ID', 'truelysell_core' ); ?></label>
					<img class="help_tip tips" data-tip="<?php _e( 'Optionally link this package to an order.', 'truelysell_core' ); ?>" src="<?php echo WC()->plugin_url() ?>/assets/images/help.png" height="16" width="16">
				</th>
				<td>
					<input type="number" step="1" name="order_id" id="order_id" value="<?php echo esc_attr( $order_id ); ?>" class="input-text regular-text" placeholder="<?php _e( 'N/A', 'truelysell_core' ); ?>" />
				</td>
			</tr>
		</table>
		<p class="submit">
			<input type="hidden" name="package_id" value="<?php echo esc_attr( $this->package_id ); ?>" />
			<input type="submit" class="button button-primary" name="save_package" value="<?php _e( 'Save Package', 'truelysell_core' ); ?>" />
		</p>
		<?php
	}
	...
	public function save() {
		...
		try {
			...
			if ( $this->package_id ) {
				...
				do_action( 'truelysell_core_admin_updated_package', $this->package_id );
			} else {
				$wpdb->insert(
					"{$wpdb->prefix}truelysell_core_user_packages",
					array(
				...
				do_action( 'truelysell_core_admin_created_package', $this->package_id );
			}// End if().

			echo sprintf( '<div class="updated"><p>%s</p></div>', __( 'Package successfully saved', 'truelysell_core' ) );
