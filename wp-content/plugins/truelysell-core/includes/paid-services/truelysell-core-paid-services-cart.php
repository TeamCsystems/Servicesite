 ...
 * Cart
 */
class Truelysell_Core_Paid_Properties_Cart {
	...
	public function legacy_order_item_meta( $item_id, $values ) {
		...
		if ( isset( $values['listing_id'] ) ) {
			...
			wc_add_order_item_meta( $item_id, __( 'Listing title', 'truelysell_core' ), $listing->post_title );
	...
	public function checkout_create_order_line_item( $order_item, $cart_item_key, $cart_item_data, $order ) {
		if ( isset( $cart_item_data['listing_id'] ) ) {
			...
			$order_item->update_meta_data( __( 'Listing title', 'truelysell_core' ), $listing->post_title );
	...
	public function get_item_data( $data, $cart_item ) {
		if ( isset( $cart_item['listing_id'] ) ) {
			...
			$data[] = array(
				'name'  => __( 'Listing title', 'truelysell_core' ),
				'value' => $title,
			);
		}
...
Truelysell_Core_Paid_Properties_Cart::get_instance();
