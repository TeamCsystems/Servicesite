 ...
 *  class
 */
class Truelysell_Core_Paid_Properties {
	...
	public function __construct() {
		...
		/* Hooks */
		add_action( 'woocommerce_product_options_general_product_data', array( $this,  'truelysell_core_add_custom_settings' ) );
		...
		/* Includes */
		include_once( 'truelysell-core-paid-services-orders.php' );
		include_once( 'truelysell-core-paid-services-package.php' );
		include_once( 'truelysell-core-paid-services-cart.php' );
	...
	function truelysell_core_add_custom_settings() {
	    ...
	    // Create a number field, for example for UPC
	     woocommerce_wp_text_input( array(
			'id' 				=> '_listing_limit',
			'label' 			=> __( 'Listing limit', 'truelysell_core' ),
			'description' 		=> __( 'The number of listings a user can post with this package.', 'truelysell_core' ),
			'value' 			=> ( $limit = get_post_meta( $post->ID, '_listing_limit', true ) ) ? $limit : '',
			'placeholder' 		=> __( 'Unlimited', 'truelysell_core' ),
			'type' 				=> 'number',
			'desc_tip' 			=> true,
	    ...
	    woocommerce_wp_text_input( array(
			...
			'label' 			=> __( 'Listing duration', 'truelysell_core' ),
			'description' 		=> __( 'The number of days that the listing will be active.', 'truelysell_core' ),
		...
		///	'id' => '_listing_featured',
			///'label' => __( 'Feature Listing?', 'truelysell_core' ),
			///'description' => __( 'Feature this listing - it will have a badge and sticky status.', 'truelysell_core' ),
		...
		//woocommerce_wp_checkbox( array(
			...
			//'label' => __( 'Booking Module', 'truelysell_core' ),
			//'description' => __( 'Allow booking on listings bought from this package.', 'truelysell_core' ),
			//'value' => get_post_meta(  $post->ID, '_package_option_booking', true ),
		//) ); 

		woocommerce_wp_checkbox( array(
			'id' => '_package_option_reviews',
			'label' => __( 'Reviews Module', 'truelysell_core' ),
			'description' => __( 'Allow reviews on listings bought from this package.', 'truelysell_core' ),
		...
		woocommerce_wp_checkbox( array(
			...
			'label' => __( 'Gallery Module', 'truelysell_core' ),
			'description' => __( 'Allow gallery on listings bought from this package.', 'truelysell_core' ),
			'value' => get_post_meta(  $post->ID, '_package_option_gallery', true ),
		) ); 

	    woocommerce_wp_text_input( array(
			'id' 				=> '_package_option_gallery_limit',
			'label' 			=> __( 'Gallery module images limit', 'truelysell_core' ),
			'description' 		=> __( 'Limit the number of images that can be uploaded. Set to empty or 0 set no limit', 'truelysell_core' ),
		...
		///	'id' => '_package_option_social_links',
			///'label' => __( 'Social Links Module', 'truelysell_core' ),
			///'description' => __( 'Allow social links to be displayed on the listings bought from this package.', 'truelysell_core' ),
			///'value' => get_post_meta(  $post->ID, '_package_option_social_links', true ),
	///	) );

		woocommerce_wp_checkbox( array(
			'id' => '_package_option_opening_hours',
			'label' => __( 'Opening Hours Module', 'truelysell_core' ),
			'description' => __( 'Allow Opening Hours widget to be displayed on the listings bought from this package.', 'truelysell_core' ),
			'value' => get_post_meta(  $post->ID, '_package_option_opening_hours', true ),
		) );
		
		woocommerce_wp_checkbox( array(
			'id' => '_package_option_video',
			'label' => __( 'Video Module', 'truelysell_core' ),
			'description' => __( 'Allow Video widget to be displayed on the listings bought from this package.', 'truelysell_core' ),
			'value' => get_post_meta(  $post->ID, '_package_option_video', true ),
		) );		
		///woocommerce_wp_checkbox( array(
			///'id' => '_package_option_coupons',
			///'label' => __( 'Coupons Module', 'truelysell_core' ),
			///'description' => __( 'Allow Coupons widget to be displayed on the listings bought from this package.', 'truelysell_core' ),
...
new Truelysell_Core_Paid_Properties();
