 ...
 */
class Truelysell_Core_Admin_Edit_Listing_Package {
	...
	public function __construct() {
		...
		if ( ! empty( $_POST['save_listing_package'] ) && ! empty( $_POST['truelysell_core_paid_listings_package_editor_nonce'] ) && wp_verify_nonce( $_POST['truelysell_core_paid_listings_package_editor_nonce'], 'save' ) ) {
			$this->save();
		}
	}
	...
	public function form() {
		...
		<table class="form-table">
			<h2><?php 
		    	...
		    	$user_package = get_post_meta($this->listing_id,'_user_package_id',true);
		    		...
		    		$package = truelysell_core_get_package_by_id($this->package_id);	
			...
			<tr>
				<th>
					<label for="_user_package_id"><?php _e( 'Assigned to Listing Package', 'truelysell_core' ); ?></label><br>
					
				</th>
				<td>
					<select name="_user_package_id">
					<?php echo truelysell_core_available_packages($post_author_id,$user_package); ?>	
					</select>
		...
		<p class="submit">
			...
			<input type="submit" class="button button-primary" name="save_listing_package" value="<?php _e( 'Save Package', 'truelysell_core' ); ?>" />
		</p>
		<?php
	}
	...
	public function save() {
		...
		try {
			...
			if ( $current_package_id != $new_package_id) {
					...
					truelysell_core_increase_package_count($post_author_id, $new_package_id);
					if($decrease == 1) {
					
						truelysell_core_decrease_package_count($post_author_id, $current_package_id);
					}
				echo sprintf( '<div class="updated"><p>%s</p></div>', __( 'Package successfully changed', 'truelysell_core' ) );

			} else {
			echo sprintf( '<div class="updated"><p>%s</p></div>', __( 'You haven\'t changed package', 'truelysell_core' ) );
			}// End if().
