 ...
 */
class Truelysell_Core_Admin_Packages_Listings extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			case 'listing_id' :
				...
				return $product ? '<a href="' . admin_url( 'post.php?post=' . absint( $item  ) . '&action=edit' ) . '">' . esc_html( get_the_title($item ) ) . '</a>' : __( 'n/a', 'truelysell_core' );
			...
			case 'user_package':
		    	...
		    	if($user_package){
		    		$package = truelysell_core_get_package_by_id($user_package);	
			...
			case 'listing_actions' :
				...
				), admin_url( 'admin.php?page=truelysell_core_paid_listings_package_editor' ) ) );
				return '<div class="actions">' .
					'<a class="button button-icon icon-edit" href="' . $edit_url . '">' . __( 'Change package', 'truelysell_core' ) . '</a>' .
					'</div>' .
					'</div>';
		}// End switch().
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'user_package'      => __( 'Package', 'truelysell_core' ),
    ...
    $this->search_box( __( 'Search Listings' ), 'search-truelysell_core_paid_listings_package_editor' ); 
    ?>
