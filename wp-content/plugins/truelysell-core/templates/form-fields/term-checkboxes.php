...
$dynamic_features = (get_option('truelysell_dynamic_features') == 'on') ? 'dynamic' : '' ;
if($dynamic_features == 'dynamic'){ ?>
	<div class="<?php echo esc_attr($dynamic_features); ?> checkboxes in-row truelysell_core-term-checklist truelysell_core-term-checklist-<?php echo $key ?>">
	<div class="notification warning"><p><?php esc_html_e('Please choose category to display available features','truelysell_core') ?></p> </div>
</div>
<?php } else { ?>
	<div class="<?php echo esc_attr($dynamic_features); ?> checkboxes in-row truelysell_core-term-checklist truelysell_core-term-checklist-<?php echo $key ?>">
	<?php
