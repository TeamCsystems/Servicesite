...
<div class="nothing-compares-2u" <?php if(!empty($listings)){ ?> style="display:none;" <?php } ?>>
	<section id="listings-not-found" class="margin-bottom-50">
		<h2><?php esc_html_e('No listings to compare','truelysell_core'); ?> </h2>
		<p><?php _e( 'Looks like you haven\'t add anything to compare yet!', 'truelysell_core' ); ?></p>
	</section>
	<?php 
    ...
    if($wp_query->have_posts()) { ?>
	    <h3 class="desc-headline no-border margin-bottom-35 margin-top-60 print-no"><?php esc_html_e('Check Other Properties','truelysell_core'); ?></h3>
	     <div class="carousel">
			<?php 
	        if ( $wp_query->have_posts() ) {
