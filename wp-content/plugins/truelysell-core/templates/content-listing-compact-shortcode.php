...
<div class="listing-item compact"  <?php truelysell_get_geo_data($post); ?> >
	<?php if(truelysell_core_is_featured($post->ID)){ ?>
					<i class="featured-icon tooltip right" title="<?php echo esc_attr_e('Featured Listing','truelysell_core') ?>"></i>
				<?php } ?>
	<a href="<?php the_permalink(); ?>" class="listing-img-container">

		<div class="listing-badges">
			<?php if(truelysell_core_is_featured( $post->ID )) : ?><span class="featured"><?php esc_html_e('Featured','truelysell_core'); ?></span><?php endif; ?>
			...
			<a href="<?php the_permalink(); ?>" class="listing-item-container compact">
				<div class="listing-item">
					...
					<?php if(get_post_meta( $post->ID,'_opening_hours_status',true )) { 
			                if( truelysell_check_if_open() ){ ?>
			                    <div class="listing-badge now-open"><?php esc_html_e('Now Open','truelysell_core'); ?></div>
			                <?php } else { 
			                    if( truelysell_check_if_has_hours() ) { ?>
			                        <div class="listing-badge now-closed"><?php esc_html_e('Now Closed','truelysell_core'); ?></div>
			                    <?php } ?>
			            <?php } 
			        }?>
