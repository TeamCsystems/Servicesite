...
(function ( $ ) {
	...
	$('.truelysell-custom-image-upload').on('click', function( event ){
		var to = $(this);
		event.preventDefault();
...
// $('.truelysell_map_settings').hide();
// $('.truelysell_map_settings.map_provider').cha();
$('.truelysell_map_settings.map_provider input[type=radio]').on('change', function() {
     switch($(this).val()) {
         case 'osm':
             	$('.truelysell_map_settings.maps_api').hide();
             	$('.truelysell_map_settings.mapbox_access_token').hide();
             	$('.truelysell_map_settings.mapbox_style_url').hide();
             	$('.truelysell_map_settings.mapbox_retina').hide();
				$('.truelysell_map_settings.bing_maps_key').hide();
				$('.truelysell_map_settings.thunderforest_api_key').hide();
				$('.truelysell_map_settings.here_app_id').hide();
             	$('.truelysell_map_settings.here_app_code').hide();
             break;
         case 'google':
	            $('.truelysell_map_settings.maps_api').show();
             	$('.truelysell_map_settings.mapbox_access_token').hide();
             	$('.truelysell_map_settings.mapbox_style_url').hide();
				$('.truelysell_map_settings.bing_maps_key').hide();
				$('.truelysell_map_settings.mapbox_retina').hide();
             ...
             break;
         case 'mapbox':
             $('.truelysell_map_settings.mapbox_access_token').show();
             $('.truelysell_map_settings.mapbox_retina').show();
             $('.truelysell_map_settings.mapbox_style_url').show();
             $('.truelysell_map_settings.maps_api').hide();
             ...
             break;         
        case 'bing':
             	$('.truelysell_map_settings.bing_maps_key').show();
             	$('.truelysell_map_settings.maps_api').hide();
             	$('.truelysell_map_settings.mapbox_retina').hide();
             	$('.truelysell_map_settings.mapbox_access_token').hide();
             	$('.truelysell_map_settings.mapbox_style_url').hide();
             ...
             break;       
        case 'thunderforest':
             	$('.truelysell_map_settings.thunderforest_api_key').show();
             	$('.truelysell_map_settings.maps_api').hide();
             	$('.truelysell_map_settings.mapbox_retina').hide();
             	$('.truelysell_map_settings.mapbox_access_token').hide();
             	$('.truelysell_map_settings.mapbox_style_url').hide();
             ...
             break;        
        case 'here':
             	$('.truelysell_map_settings.here_app_id').show();
             	$('.truelysell_map_settings.here_app_code').show();
             	$('.truelysell_map_settings.maps_api').hide();
             	$('.truelysell_map_settings.mapbox_retina').hide();
             	$('.truelysell_map_settings.mapbox_access_token').hide();
             	$('.truelysell_map_settings.mapbox_style_url').hide();
...
$('.truelysell_map_settings.map_provider input[type=radio]:checked').trigger('change');

 $('#truelysell_core_booking_metabox')
    .on( 'init', function() {
  ...
  $('.cmb-type-truelysell-calendar #truelysell-calendar-outer-container').on("click", 'span.calendar-day-date', function(e) { 
      e.preventDefault();
      var td = $(this).parent();
  ...
  $('.cmb-type-truelysell-calendar #truelysell-calendar-outer-container').on("click", 'button', function(e) { 
    e.preventDefault();
  ...
  $('.cmb-type-truelysell-calendar #truelysell-calendar-outer-container').on("click", '.prev', function(event) { 
      var month =  $(this).data("prev-month");
      var year =  $(this).data("prev-year");
      getCalendar(month,year);
  });

  $('.cmb-type-truelysell-calendar #truelysell-calendar-outer-container').on("click", '.next', function(event) { 
  	
      var month =  $(this).data("next-month");
      var year =  $(this).data("next-year");
      getCalendar(month,year);
  });
  $('.cmb-type-truelysell-calendar #truelysell-calendar-outer-container').on("blur", '#currentYear', function(event) { 
...
function show_hide_paypal_payout_all_inputs(){
	if ($(pp_activate_elem).val() === 'no'){
		$('#payout_environment').parents('tr.truelysell_settings_select').hide();

		$('#payout_sandbox_client_id').parents('tr.truelysell_settings_text').hide();
		$('#payout_sandbox_client_secret').parents('tr.truelysell_settings_password').hide();

		$('#payout_live_client_id').parents('tr.truelysell_settings_text').hide();
		$('#payout_live_client_secret').parents('tr.truelysell_settings_password').hide();

		$('#payout_email_subject').parents('tr.truelysell_settings_textarea').hide();
		$('#payout_email_message').parents('tr.truelysell_settings_textarea').hide();
		$('#payout_trx_note').parents('tr.truelysell_settings_textarea').hide();
	}else{
		$('#payout_environment').parents('tr.truelysell_settings_select').show();
		show_hide_paypal_payout_api_inputs();
		$('#payout_email_subject').parents('tr.truelysell_settings_textarea').show();
		$('#payout_email_message').parents('tr.truelysell_settings_textarea').show();
		$('#payout_trx_note').parents('tr.truelysell_settings_textarea').show();
	}
}
...
function show_hide_paypal_payout_api_inputs(){
	...
	if (pp_payout_env === 'live'){
		...
		// show live inputs
		$('#payout_live_client_id').parents('tr.truelysell_settings_text').show();
		$('#payout_live_client_secret').parents('tr.truelysell_settings_password').show();
		...
		// show sandbox inputs
		$('#payout_sandbox_client_id').parents('tr.truelysell_settings_text').show();
		$('#payout_sandbox_client_secret').parents('tr.truelysell_settings_password').show();
	}
}
...
$('.cancel-payout-button').click(function (e){
	...
	if (window.confirm(truelysell_admin.pp_cancel_payout_confirmation_msg)) {
		let $this = $(this);

		$this.parents('#truelysell-make-payout').find('.loader-div').removeClass('truelysell-hidden');

		var data = {
			'action': 'truelysell_cancel_payout_request',
			'pp_item_id': $($this).attr('data-pp_item_id'),
			'commission_id': $($this).attr('data-commission_id'),
		};

		jQuery.post(truelysell_admin.ajax_url, data, function(response) {
			if (response.data.success === true){
				// refresh the page. So, the admin can see the changes.
				window.location = window.location.href;
			}else{
				alert(response.data.msg);
				$($this).parents('#truelysell-make-payout').find('.loader-div').addClass('truelysell-hidden');
			}
		});
	}
})

let truelysell_payout_check_boxes_td = $('#truelysell-make-payout').find('table tbody tr > td:first-child');
let truelysell_payout_checkboxes = $(truelysell_payout_check_boxes_td).find('input[type=checkbox]');

$(truelysell_payout_checkboxes).change(function () {
	let $this = $(this);
	truelysell_check_cbs();
});

truelysell_check_cbs();
function truelysell_check_cbs(){
	let is_any_cb_checked = false;
	$(truelysell_payout_check_boxes_td).each(function (i, e) {
		let cb = $(e).find('input[type=checkbox]');
		if ($(cb).is(':checked')){
			is_any_cb_checked = true;
			truelysell_remove_payout_class();
		}
	});

	if (! is_any_cb_checked){
		truelysell_add_payout_class();
	}
}
function truelysell_remove_payout_class(){
	$('#truelysell-make-payout').find('input[name="submit_new_payout"]').removeClass('truelysell_disable').attr('disabled', false);
}

function truelysell_add_payout_class(){
	$('#truelysell-make-payout').find('input[name="submit_new_payout"]').addClass('truelysell_disable').attr('disabled', true);
}

// Truelysell PayOut - Ends
