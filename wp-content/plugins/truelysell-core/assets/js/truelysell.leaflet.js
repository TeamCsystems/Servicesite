...
$(document).ready(function(){
  ...
  function locationData(locationURL,locationImg,locationTitle,locationType, locationPrice, locationAddress, locationRating, locationRatingCounter) {
               ...
               if(locationType == 'classifieds') {
                ...
                // Rules
                    ...
                    if (locationRating >= 4.75) {
                      ...
                      output_bottom = '<div class="'+infoBox_ratingType+'" data-rating="'+locationRating+'"><div class="rating-counter">('+locationRatingCounter+' '+truelysell_core.maps_reviews_text+')</div>'+stars+'</div>'+
                         '</div>'+
                      '</div>';
                  } else {
                    output_bottom = '<div class="'+infoBox_ratingType+'"><span class="not-rated">'+truelysell_core.maps_noreviews_text+'</span></div>'+
                         '</div>'+
                      '</div>';
                  }
               }
     ...
     window.L_DISABLE_3D = true 
      ...
      var latlngStr = truelysell_core.centerPoint.split(",",2);
      ...
      if (typeof(_map) != 'undefined' && _map != null) {
          ...
          switch (truelysell_core.map_provider) {
            case "osm":
              L.tileLayer(
            ...
            case "mapbox":
              var accessToken = truelysell_core.mapbox_access_token;
              var mapbox_style_url = truelysell_core.mapbox_style_url;

              if (truelysell_core.mapbox_retina) {
            ...
            case "bing":
              var options = {
                bingMapsKey: truelysell_core.bing_maps_key,
            ...
            case "thunderforest":
              var tileUrl =
                  ...
                  truelysell_core.thunderforest_api_key,
            ...
            case "here":
              L.tileLayer
                .here({
                  appId: truelysell_core.here_app_id,
                  appCode: truelysell_core.here_app_code,
                })
                .addTo(map);
              break;
          }
    ...
    function listingsMap(map){
        ...
        for (var i = 0; i < locations.length; i++) {
          ...
          var truelysellIcon = L.divIcon({
              iconAnchor: [20, 51], // point of the icon which will correspond to marker's location
              popupAnchor: [0, -51],
              className: 'truelysell-marker-icon',
        ...
        if(markerArray.length > 0 ){
          ...
          if(truelysell_core.maps_autofit == 'on'){
    ...
    $( '#truelysell-listings-container' ).on( 'update_results_success', function (  ) {
        if(window.map){
            map.closePopup();
    ...
    function submitPropertyMap(){
        ...
        if(truelysell_core.submitCenterPoint) {
          var latlngStr = truelysell_core.submitCenterPoint.split(",",2);
          var lat = parseFloat(latlngStr[0]);
          var lng = parseFloat(latlngStr[1]);
        ...
        switch(truelysell_core.map_provider) {
          ...
          case 'bing':
              L.tileLayer.bing(truelysell_core.bing_maps_key).addTo(submit_map)
          break;

          case 'thunderforest':
              var tileUrl = 'https://tile.thunderforest.com/cycle/{z}/{x}/{y}{r}.png?apikey='+truelysell_core.thunderforest_api_key,
              layer = new L.TileLayer(tileUrl, {maxZoom: 18});
              submit_map.addLayer(layer);
          break;

          case 'here':
              L.tileLayer.here({appId: truelysell_core.here_app_id, appCode: truelysell_core.here_app_code}).addTo(submit_map);
          break;
        }
        ...
        if(truelysell_core.address_provider == 'osm'){
          if(truelysell_core.country){
            var geocoder = new L.Control.Geocoder.Nominatim( {
                geocodingQueryParams: {countrycodes: truelysell_core.country} //accept-language: 'en'
            });
          } else {
            var geocoder = new L.Control.Geocoder.Nominatim();  
          } 
    ...
    function singleListingMap() {
        ...
        var mapOptions = {
            center: [lat,lng],
            zoom: truelysell_core.maps_single_zoom,
            zoomControl: false,
            gestureHandling: true
         }
        ...
        switch(truelysell_core.map_provider) {
         ...
         case 'google':
          ...
          case 'bing':
              L.tileLayer.bing(truelysell_core.bing_maps_key).addTo(map_single)
          break;
          ...
          case 'here':
              L.tileLayer.here({appId: truelysell_core.here_app_id, appCode: truelysell_core.here_app_code}).addTo(map_single);
          break;
        }
...
if(truelysell_core.address_provider == 'osm'){
        if(truelysell_core.country){
          var geocoder = new L.Control.Geocoder.Nominatim( {
              geocodingQueryParams: {countrycodes: truelysell_core.country}
          });
        } else {
          var geocoder = new L.Control.Geocoder.Nominatim();  
        } 
        ...
        $("#location_search").on("focusin",function() {
            ...
            if( $(this).val() == 0 ) {
              $('div#truelysell-listings-container' ).triggerHandler( 'update_results', [ 1, false ] );
            }
        });
        ...
        $("#location_search").on("keydown",function search(e) {
         ...
         $("#truelysell_core-search-form").on( "click", "#leaflet-geocode-cont ul li", function(e) {
        ...
        $('#truelysell_core-search-form').on('submit', function(){
      ...
      $(".geoLocation, #truelysell_core-search-form .location a,.main-search-input-item.location a,.form-field-_address-container a").on("click", function (e) {
          e.preventDefault();
                  ...
                  if(truelysell_core.address_provider == 'google'){
                    ...
                    if(window.map){
                      ...
                      geocoder.reverse(latlng, map.options.crs.scale(map.getZoom()), function(results) { 
                        ...
                        map.panTo(newLatLng);
                        var listing_results      = $('#truelysell-listings-container');
      ...
      if(truelysell_core.maps_autolocate){
        
        $(".geoLocation, #truelysell_core-search-form .location a,.main-search-input-item.location a").trigger('click')
      }
  });
})(this.jQuery);
