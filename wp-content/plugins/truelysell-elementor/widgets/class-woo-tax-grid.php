...
class WooTaxonomyGrid extends Widget_Base
{
	...
	{
		return 'truelysell-woo-taxonomy-grid';
	}
	...
	{
		return __('Shop Taxonomy Grid', 'truelysell_elementor');
	}
	...
	protected function _register_controls()
	{
		...
		foreach ($taxonomy_names as $key => $value) {
			...
			$this->add_control(
				$value->name . '_include',
				[
					'label' => __('Include products from ' . $value->label, 'truelysell_elementor'),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
			...
			$this->add_control(
				$value->name . '_exclude',
				[
					'label' => __('Exclude products from ' . $value->label, 'truelysell_elementor'),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
		...
		$this->add_control(
			'show_counter',
			[
				'label' => __('Show products counter', 'truelysell_elementor'),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => __('Show', 'your-plugin'),
				'label_off' => __('Hide', 'your-plugin'),
				'return_value' => 'yes',
...
?>
			<div class="truelysell-woo-taxonomy-boxes">
					...
					<a href="<?php echo get_term_link($term); ?>" class="photo-box ">
						...
						<div class="photo-box-content">
							<?php if ($term) : ?>
								...
								<?php if ($settings['show_counter']) : ?><span><?php echo $term->count; ?> <?php esc_html_e('Products', 'truelysell_elementor') ?></span> <?php endif; ?>
							<?php endif; ?>
						</div>
					</a>
