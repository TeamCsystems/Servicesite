...
class ImageBox extends Widget_Base {
	...
	public function get_title() {
		return __( 'Truelysell Category Box', 'truelysell_elementor' );
	}
	...
	protected function _register_controls() {
		...
		// 	'featured',
		// 	[
		// 		'label' => __( 'Featured badge', 'truelysell_elementor' ),
		...
		foreach ($taxonomy_names as $key => $value) {
			...
			$this->add_control(
				$value->name.'term',
				[
					'label' => __( 'Show term from '.$value->label, 'truelysell_elementor' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'default' => [],
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'alternative-imagebox' => __( 'Alternative', 'truelysell_elementor' ),
				],
			]
		);
			...
			<a <?php echo 'href="' . esc_url($url) . '"' . $target . $nofollow; ?> class="img-box " 
				...
				<div class="img-box-content visible">
					<?php if($term): ?>
						...
						<?php if($settings['show_counter']) : ?><span><?php  printf( _n( '%s Listing', '%s Listings', $term->count, 'truelysell_elementor' ), $term->count ); ?>
							</span> <?php endif; ?>
					<?php endif; ?>
				</div>
			</a>
