...
class TaxonomyList extends Widget_Base {
	...
	public function get_name() {
		return 'truelysell-taxonomy-list';
	}
	...
	public function get_title() {
		return __( 'Truelysell Taxonomy List', 'truelysell_elementor' );
	}
...
// 	'taxonomy' => '',
   ...
   //          'autoplayspeed'      => '3000',
		...
		$this->add_control(
			'type',
			[
				'label' => __( 'List Type ', 'truelysell_elementor' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => '',
				'options' => [
					'group_by_parents' => __( 'Group by parents', 'truelysell_elementor' ),
					'all' => __( 'Show all categories', 'truelysell_elementor' ),
					'only_parents' => __( 'Show only parent categories', 'truelysell_elementor' ),
					'parent' => __( 'Show only child categories from single parent', 'truelysell_elementor' ),
		...
		// 		$value->name.'_include',
		// 		[
		// 			'label' => __( 'Parent category from '.$value->label, 'truelysell_elementor' ),
		...
		foreach ($taxonomy_names as $key => $value) {
			...
			$this->add_control(
				$value->name.'_include',
				[
					'label' => __( 'Include '.$value->label, 'truelysell_elementor' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
			...
			$this->add_control(
				$value->name.'_exclude',
				[
					'label' => __( 'Exclude '.$value->label, 'truelysell_elementor' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => true,
					'default' => [],
			...
			$this->add_control(
				...
				[
					'label' => __( 'Parent category '.$value->label, 'truelysell_elementor' ),
					'type' => Controls_Manager::SELECT2,
					'label_block' => true,
					'multiple' => false,
					'default' => [],
		...
		$this->add_control(
			...
			[
				...
				'default' => 'name',
				'options' => [
					'name' =>  __( 'Name', 'truelysell_elementor' ),
					'term_id' =>  __(  'Order by term id. ', 'truelysell_elementor' ),
					'term_order' =>  __(  'Order by term order. ', 'truelysell_elementor' ),
					'count'=>  __(  'Order by number of listings assinged.', 'truelysell_elementor' ),
					'include' =>  __(  'Match the order of the "Include listing" param.', 'truelysell_elementor' ),
				],
			]
		);
		$this->add_control(
		...
		$this->add_control(
			'hide_empty',
			[ 
				'label' => __( 'Hide empty categories', 'truelysell_elementor' ),
      	...
      	if($settings['hide_empty'] == 'yes' ){
       	...
       	if($type == 'all') {
	        ...
	        if ( !is_wp_error( $categories ) ) {
	            ...
	            $chunks = truelysell_partition($categories, 3);
	            foreach ($chunks as $chunk) {
	                $output .= '<div class="col-md-3">
	                        ...
	                        foreach ($chunk as $term) {
								...
								$count = truelysell_get_term_post_count($settings['taxonomy'],$term->term_id);
	                        	...
	                        	$output .= '<li class="child-listing-category">
									...
									if (!empty($_icon_svg_image)) {
										$output .= '<i class="child-category-icon truelysell-svg-icon-box-grid">'.truelysell_render_svg_icon($_icon_svg).'</i>';
					          		 } else { 
					          			if($icon != 'emtpy') {
	                    ...
	                    <div class="row">
	                        <div class="col-md-3"><h4 class="parent-listings-category">';
	                         ...
	                         $count = truelysell_get_term_post_count($settings['taxonomy'],$term->term_id);
								...
								<div class="child-category-icon-container">';
									if (!empty($_icon_svg_image)) {
										$output .= '<i class="parent-category-icon truelysell-svg-icon-box-grid">'.truelysell_render_svg_icon($_icon_svg).'</i>';
					          		 } else { 
					          			if($icon != 'emtpy') {
					          				$check_if_im = substr($icon, 0, 3);
						                    if($check_if_im == 'im ') {
	                            ...
	                            $chunks = truelysell_partition($subterms, 3);
	                            foreach ($chunks as $chunk) {
	                                $output .= '<div class="col-md-3">
	                                        <ul>';
								...
								$count = truelysell_get_term_post_count($settings['taxonomy'],$subterms->term_id);
