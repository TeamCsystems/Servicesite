...
class IconBox extends Widget_Base {
	...
	public function get_title() {
		return __( 'Truelysell Icon Box', 'truelysell_elementor' );
	}
	...
	// 	    'with_line' 	=> 'yes',
		...
		$this->add_control(
			...
			array(
				...
				'default' => __( 'Content', 'truelysell_elementor' ),
			)
		);	
		$this->add_control(
			'url',
		...
		$this->add_control(
			...
			[
				...
				'default' => 'top',
				'options' => [
					'top' =>  __( 'Icon on top', 'truelysell_elementor' ),
					'next' =>  __(  'Icon on the right. ', 'truelysell_elementor' ),
		...
		$this->add_control(
			'icon',
			[
				'label' => __( 'Icon', 'truelysell_elementor'),
				...
				echo '<a href="' . $settings['url']['url'] . '"' . $target . $nofollow . '>';	
				 ...
				 <div class="icon-box-2 ">
				 		<?php if(isset($settings['icon']['library']) && $settings['icon']['library'] == 'svg') { ?>
				 			<i class="truelysell-svg-icon-box"><?php \Elementor\Icons_Manager::render_icon( $settings['icon'], [ 'aria-hidden' => 'true' ] );  ?></i>
				 		<?php } else {
