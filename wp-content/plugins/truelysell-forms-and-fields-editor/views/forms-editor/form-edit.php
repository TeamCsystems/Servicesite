<div class="edit-form-field " style="display:none">
        ...
        <?php } else if(substr($field['name'], 0, 3) == 'tax' || in_array($field['type'], array('multi-checkbox', 'multi-checkbox-row', 'select') ) ) { ?>
            ...
            <p class="type-container">
                ...
                <?php } else { ?>
                    ...
                    <select class="field-type-selector" name="type[<?php echo esc_attr( $index ); ?>]">
                     <option <?php selected($field['type'], 'select-taxonomy'); ?> value="select-taxonomy1">Select Taxonomy</option>
                </select>
                    <?php } ?>
  ...
  <?php if(in_array($field['type'], array('select','select_multiple','multicheck_split'))) { ?>
            ...
            <input name="labeltext[<?php echo $index; ?>]"  type="text" value="<?php if(isset( $field['labeltext'])) {  echo esc_attr( $field['labeltext'] ); } ?>">
        </p>
        ...
        <?php if($editor_type=='truelysell-user-fields-registration') { 
            ?>
        ...
        <?php if($tab =='search_on_home_page' ) : ?>  
        <p class="css-class-container">
            <label for="label">Icon Class</label>
            <input name="icon_class[<?php echo $index; ?>]"  type="text" value="<?php if(isset( $field['icon_class'])) {  echo esc_attr( $field['icon_class'] ); } ?>">
        </p>
        
        <?php endif; ?>
        <?php if(in_array($field['type'], array('text','texticon','location','locationhome','select','hidden'))) {  ?>
        <p class="css-class-container">
        ...
        <?php if(in_array($field['type'], array( 'texticon','locationhome'))) {  ?>
            <?php if ($tab == 'search_on_home_page')  {  ?>

                <p class="class-container">
                <label for="label">Style<span class="dashicons dashicons-editor-help" title="Field’s width using Bootstrap columns"></span> </label>
                <select class="field-edit-class-select" name="css_class[<?php echo esc_attr($index); ?>]">
                     <option value="style1" <?php if($field['css_class']=='style1') { echo 'selected';} ?>>Style 1</option>
                    <option value="style2" <?php if($field['css_class']=='style2') { echo 'selected';} ?>>Style 2</option>
                </select>
            </p>
                <?php } ?>
